...
/home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/parse_options.sh
...
function filter_file {
  ...
  /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/filter_scp.pl $filter ${file_to_filter}.tmp > $file_to_filter
...
function filter_speakers {
  ...
  /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/utt2spk_to_spk2utt.pl $data/utt2spk > $data/spk2utt
  ...
  /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/spk2utt_to_utt2spk.pl $data/spk2utt > $data/utt2spk
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang $maybe_wav $maybe_utt2dur $maybe_utt2num_frames; do
    if [ -f $data/$x ]; then
      /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/filter_scp.pl $data/$x $tmpdir/utts > $tmpdir/utts.tmp
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames $maybe_wav $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
      if ! cmp -s $data/$x <( /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/filter_scp.pl $tmpdir/utts $data/$x ) ; then
       /home/kaldi_asr/kaldi/egs/voxceleb/v2/utils/filter_scp.pl $tmpdir/utts $data/.backup/$x > $data/$x
      fi
    fi
  done
